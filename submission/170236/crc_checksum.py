...
def unstuff(dataList):
	...
	for itr in range(len(dataList)):
		if counter==1:
			# print("---------------")
			# print(dataList[itr])
			# print(int(dataList[itr],16))
			# print(bin(int(dataList[itr],16)))
			# print(str(bin(int(dataList[itr],16))))
			# print(format(int(str('0x20'),16),'010b'))
			# print(str(hex(int(nxor(str(bin(int(dataList[itr],16)))[2:],str(format(int(str('0x20'),16),'010b'))[2:]),2))))
			unstuffedData.append(str(hex(int(nxor(str(bin(int(dataList[itr],16)))[2:],str(format(int(str('0x20'),16),'010b'))[2:]),2))))
			counter=0
			# print("---------------")
			continue
		if dataList[itr] == ESC:
			# print(dataList[itr]," Found ESC")
			counter=1
			continue
		else:
			# print(dataList[itr],"    NORMAL BITS")
...
for i in range(len(ValidFrameSet)):
	...
	# for j in range(0,len(DataStream)-1,ByteLength):
	# 	print(DataStream[j:j+ByteLength])
	ValidHexData = [str(format(int((DataStream[j:j+ByteLength]),2),'#04x')) for j in range(0, len(DataStream)-ByteLength, ByteLength)]
	unstuffedDataList=unstuff(ValidHexData)
	# print(ValidHexData)
	for j in  range(len(unstuffedDataList)):
		# print(str(unstuffedDataList[j]),"      ",str(int(str(unstuffedDataList[j]),16)))
